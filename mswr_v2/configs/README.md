...
| `robust_mrae_shortcos.yaml` | Overfit attack: byte-identical to `robust_mrae_spectralffn.yaml` except `end_epoch: 140` (was 220), i.e. a shorter cosine that anneals to `min_lr` through the ~ep70 overfit window. Single-variable A/B (schedule only) vs `robust_mrae_spectralffn.yaml`. |
