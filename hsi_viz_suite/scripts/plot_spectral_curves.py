...
class SpectralAnalyzer:
    def __init__(self, results_dir: str, output_dir: str, dpi: int = 300) -> None:
        ...
        self.dpi = dpi
        self.wavelength_range: Tuple[int, int] = (400, 700)
        self.n_bands = 31
    ...
    def analyze_spectral_signatures(
        self, sample: str, pixel_locations: List[Tuple[int, int]]
    ) -> None:
        """Create a spectral signature plot for selected pixels."""
        ...
        self._plot_individual(ax1, pred, tgt, pixel_locations[:5])
        ax4 = fig.add_subplot(gs[1, 1])
        self._plot_bandwise_errors(ax4, pred, tgt)
    ...
    def _plot_individual(
        self, ax: "Axes", pred: np.ndarray, tgt: np.ndarray, pixels: List[Tuple[int, int]]
    ) -> None:
        cmap = plt.get_cmap('tab10')
        colors = [cmap(i / max(len(pixels) - 1, 1)) for i in range(len(pixels))]
        for i, (y, x) in enumerate(pixels):
            ax.plot(
                self.wavelengths, tgt[:, y, x], '-',
                color=colors[i], label=f'GT ({x},{y})', alpha=0.85
            )
            ax.plot(
                self.wavelengths, pred[:, y, x], '--',
                color=colors[i], label=f'Pred ({x},{y})', alpha=0.85
            )
        ax.set(
            xlabel='Wavelength (nm)',
            ylabel='Reflectance',
            title='Spectral Signatures',
            xlim=self.wavelength_range,
            ylim=(0, 1),
        )
        ax.grid(True, alpha=0.3)
        ax.legend(bbox_to_anchor=(1.02, 1), loc='upper left', fontsize=8)

    def _plot_bandwise_errors(self, ax: "Axes", pred: np.ndarray, tgt: np.ndarray) -> None:
        # Use robust MRAE: mask out near-zero target values
        tgt_abs = np.abs(tgt)
        mask = tgt_abs > 1e-3  # Only compute where signal is strong
        ratio = np.abs(pred - tgt) / (tgt_abs + EPS)
        # Compute per-band MRAE only on valid pixels
        mrae = np.array([
            ratio[c][mask[c]].mean() if mask[c].any() else 0.0
            for c in range(pred.shape[0])
        ])
        rmse = np.sqrt(np.mean((pred - tgt) ** 2, axis=(1, 2)))  # (C,H,W)->(C,)
        ax2 = ax.twinx()
...
def main() -> None:
    ...
    if args.pixels and len(args.pixels) % 2 == 0:
        it = iter(args.pixels)
        pixels = [(y, x) for y, x in zip(it, it)]
    else:
