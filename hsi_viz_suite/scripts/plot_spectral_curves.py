...
class SpectralAnalyzer:
    ...
    def analyze_spectral_signatures(self, sample: str, pixel_locations: List[Tuple[int, int]]) -> None:
    ...
    def _plot_individual(self, ax: plt.Axes, pred: np.ndarray, tgt: np.ndarray, pixels: List[Tuple[int, int]]) -> None:
    ...
    def _plot_bandwise_errors(self, ax: plt.Axes, pred: np.ndarray, tgt: np.ndarray) -> None:
        mrae = np.mean(np.abs(pred - tgt) / (np.abs(tgt) + EPS), axis=(1, 2))  # (C,H,W)->(C,)
