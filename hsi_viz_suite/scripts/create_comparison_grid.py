...
class ComparisonGridGenerator:
    ...
    def _load_hsi(self, dir_path: Path, sample: str):
    ...
    def create_main_comparison_figure(self, sample_names: List[str], methods: Optional[Dict[str,str]] = None):
        ...
        for r, s in enumerate(sample_names[:n_samples]):
            ...
            for c,(mname,mdir) in enumerate(methods.items()):
                ...
                rgb = hsi_to_rgb(pred, self.cmf).squeeze(0).permute(1,2,0).numpy()
